...
def process_pcap(input_file):
    ...
    for packet in packets:
        if not handshake_found and packet.haslayer(TCP) and packet[TCP].flags == "S" and packet[TCP].ack == 0:
            syn_ack_packet = None
            ack_packet = None
    ...
    if handshake_found:
        save_handshake_packets(handshake_packets, "handshake.pcap")
        print("First Three-Way Handshake saved as handshake.pcap")
    else:
        print("No Three-Way Handshake found")

# Replace 'input.pcap' with the path to your input .pcap file
process_pcap('fuzz-2006-08-23-6489.pcap')
