...
def parse_packet(packet):
    try:
        # Check if the packet is a TCP packet and contains the necessary fields for a three-way handshake
        if 'tcp' in packet and 'ip' in packet:
            #converts the TCP flags value from hexadecimal to an integer
            tcp_flags = int(packet.tcp.flags, 16)
            if tcp_flags in (2, 18, 16):
                # SYN, SYN-ACK, or ACK packet (three-way handshake)
                packet_info = {
...
packets = pcap_parser('199905150600.dump')

# Save output as JSON
output_file = 'three_way_handshakes1999.json'
