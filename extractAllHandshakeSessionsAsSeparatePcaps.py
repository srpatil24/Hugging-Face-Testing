...
def save_packets_as_pcap(packets, filename):
    output_file = output_dir + '/' + filename
    capture = pyshark.LiveCapture(output_file, display_filter='tcp')
    capture.sniff_continuously(packet_count=0)
    
    for packet in packets:
        capture.inject(packet['_ws_expert_message'])
    
    capture.close()

# Path to input pcap file
input_file = 'fuzz-2006-06-26-2594.pcap'

# Output directory for the separate pcap files
output_dir = 'pcap_output'
os.makedirs(output_dir, exist_ok=True)

# Parse the input pcap file and extract three-way handshake sessions
packets = pcap_parser(input_file)
handshake_sessions = []

# Iterate over the packets to extract three-way handshake sessions
current_session = []
for packet in packets:
    if packet is not None:
        current_session.append(packet)
        if packet['flags'] == '2' and packet['srcport'] != '443':
            # Found SYN packet, start a new session
            current_session = [packet]
        elif packet['flags'] == '18' and packet['dstport'] != '443':
            # Found SYN-ACK packet, add it to the current session
            current_session.append(packet)
        elif packet['flags'] == '16' and packet['srcport'] != '443':
            # Found ACK packet, add it to the current session and save the session
            current_session.append(packet)
            handshake_sessions.append(current_session)
            current_session = []

# Iterate over the handshake sessions and save them as separate pcap files
for session_num, handshake_packets in enumerate(handshake_sessions, start=1):
    filename = f'session_{session_num}.pcap'
    save_packets_as_pcap(handshake_packets, filename)

print("Separate pcap files saved in the 'pcap_output' directory.")